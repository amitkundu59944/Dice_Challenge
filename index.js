var randomNumber1 =Math.floor(((Math.random()*6)+1));

var randomDiceImages = "images/dice"+randomNumber1 + ".png";

document.querySelectorAll("img")[0].setAttribute("src", randomDiceImages);


var randomNumber2 =Math.floor(((Math.random()*6)+1));

var randomDiceImages2 = "images/dice"+randomNumber2 + ".png";

document.querySelectorAll("img")[1].setAttribute("src", randomDiceImages2);



 if(randomNumber1 > randomNumber2)
{
  document.querySelector("h1").innerHTML="Soham  WINS";
}
else if (randomNumber1 < randomNumber2)
{
  document.querySelector("h1").innerHTML="Amit  WINS";
}
else
{
  document.querySelector("h1").innerHTML="DRAW";
}
